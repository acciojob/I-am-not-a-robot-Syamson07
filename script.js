...
function loadImages() {
  ...
  imageSet.forEach((src, index) => {
    ...
    const baseName = src.split('.')[0]; // e.g., 'img1' from 'img1.jpg'
    img.classList.add(baseName);        // Important for Cypress test
...
function selectImage(img) {
  if (clickedImages.includes(img)) return; // prevent double-clicking the same
